...
describe('metricsServiceClick()(Component)', () => {
  let subject
  const client = { callDispatchers: jest.fn() }
  ...
  describe('with ("event", "arg1")(Component)', () => {
    ...
    it('calls client event on click', () => {
      ...
      expect(client.callDispatchers.mock.calls.length).toBe(1)
      expect(client.callDispatchers.mock.calls[0][0]).toBe('event')
      expect(client.callDispatchers.mock.calls[0][1]).toBe('arg1')
      expect(client.callDispatchers.mock.calls[0][2]).toBe('arg2')
    })
  })
})
